...
def create(fileName, *varStructs, overwrite=False, use_my_attrs=True, significant_digits=None):

    if not overwrite:
        _errorIfFileExists()

    for varStruct in varStructs:
        errorIfInvalidVarStruct(varStruct)
        errorIfInconsistentExistingVariable(fileName, varStruct)
...
def save(fileName, varStruct, overwrite=False, use_my_attrs=True, significant_digits=None):
    ...
    with nc.Dataset(fileName, 'a') as h_file:
        ...
        # create dimensions
        for name, value in zip([*dimnames, varName], [*dimvalues, varvalue]):
            if name in existed_names:
                continue
            if name in dimnames:
                h_file.createDimension(name, len(value))
                h_file.createVariable(
                    name, np.float32, (name,), significant_digits=significant_digits, compression='zlib', complevel=9, shuffle=True)
            if name == varName:
                h_file.createVariable(
...
def ncreadByDimRange(
    ...
    for minMax in minMaxs:
        ...
        for minOrMax in minMax:
            if minOrMax is not None and not isinstance(minOrMax, (int, float)):
                raise ValueError(
